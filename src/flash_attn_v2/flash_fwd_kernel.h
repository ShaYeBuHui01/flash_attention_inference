...
template <int MMA_M, class... Args, class TiledMMA>
CUTE_HOST_DEVICE auto make_tiled_copy_A_warpcontiguousM(Copy_Atom<Args...> const &copy_atom,
    ...
    auto t = make_tile(Layout<Shape<Int<AtomShape_M>, Int<kNWarps>>, Stride<_1, Int<MMAStride_M>>>{},
...
template <int MMA_M, class... Args, class TiledMMA>
CUTE_HOST_DEVICE auto make_tiled_copy_C_warpcontiguousM(Copy_Atom<Args...> const &copy_atom,
                                                        TiledMMA const &tiled_mma) {
    using TileShape_MNK = typename TiledMMA::TiledShape_MNK;
    using AtomShape_MNK = typename TiledMMA::AtomShape_MNK;
    constexpr int AtomShape_M = decltype(size<0>(AtomShape_MNK{}))::value;
...
inline __device__ void softmax_rescale_o(Tensor0 &scores, Tensor1 &scores_max, Tensor1 &scores_sum, Tensor2 &acc_o,
...
#pragma unroll
    ...
    // auto smem_thr_copy_Q = make_tiled_copy_A_warpcontiguousM<MMA_M>(typename Kernel_traits::SmemCopyAtom{},
    // tiled_mma).get_thread_slice(tidx); if (cute::thread0()) {smem_thr_copy_Q.print_all();}
...
#pragma unroll
    for (int masking_step = 0; masking_step < n_masking_steps; ++masking_step, --n_block) {
        ...
        if (masking_step > 0) {
            ...
            flash::copy<Is_even_MN, Is_even_K, /*Clear_OOB_MN=*/true>(gmem_tiled_copy_QKV, tVgV, tVsV, tKVcKV, tKVpKV,
                                                                      binfo.actual_seqlen_k - n_block * kBlockN);
        }
        cute::cp_async_fence();
        ...
        if (!Is_causal) {
            ...
            // Tensor caccS = make_identity_tensor(Shape<Int<kBlockM>, Int<kBlockN>>{});    // (BLK_M,BLK_N) ->
            // (blk_m,blk_n) Tensor taccScS = thr_mma.partition_C(caccS);                           // (MMA,MMA_M,MMA_N)
            ...
            // get<1>(stride<1>(idx_row))); } I can't get the stride from idx_row
            flash::apply_mask_causal(scores, n_block * kBlockN, binfo.actual_seqlen_k,
                                     // m_block * kBlockM + get<0>(idx_row(0)),
                                     m_block * kBlockM + (tidx / 32) * 16 + (tidx % 32) / 4, binfo.actual_seqlen_q,
                                     kNWarps * 16);
...
#pragma unroll
    ...
    // auto smem_thr_copy_O = make_tiled_copy_C_warpcontiguousM<MMA_M>(typename Kernel_traits::SmemCopyAtomO{},
    // tiled_mma).get_thread_slice(tidx);
